...
- **10·20대 탭 상단** — 카테고리 × 성별 매트릭스(10·20대 남녀가 분야별로 유독 찾는 것), 키워드별 추이 스파크라인, 관심 분야 이동(기록이 쌓일수록 정확)
- **전체 연령** (한국) — 그룹(10대~60대+, 남성·여성, 20대 여성·남성) × [7일 | 30일]: 그 그룹이 평균보다 더 관심 보인 이슈(며칠, 몇 배) · 쇼핑 관심 · 그룹 비교표

## 알림 (watchlist)

`watchlist.txt` 에 키워드를 한 줄씩 적으면 매일 반드시 추적되고, **변화가 생길 때만** 알림이 생깁니다
(단계 진입 = 확산 대기/확산 중/윗세대 상승 · 10·20대가 30대 이상보다 2배 이상 · 정점 4주 이내 예상 · 관심 키워드 첫 등장).
- 판정은 매일 새로 찍히지만 **누적 판단**(부분관측 마르코프 결정과정, `pomdp.py`)이 며칠에 걸쳐 국면 확률을 갱신 — 알림은 믿음이 동의할 때만, 추적 목록은 결정이 걸린 키워드를 붙잡음
- 사이트 **세대 확산 탭 상단 알림 카드**
- **RSS**: `https://trend.todoli.lat/feed.xml` (리더에 등록해 두면 알아서 옵니다)
- (선택) 저장소 Secret 에 `SLACK_WEBHOOK_URL` 을 넣으면 슬랙으로도 전송
